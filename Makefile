...
KIND_CLUSTER ?= daos-operator-test-e2e
...
docker-buildx: ## Build and push docker image for the manager for cross-platform support
	...
	- $(CONTAINER_TOOL) buildx create --name daos-operator-builder
	$(CONTAINER_TOOL) buildx use daos-operator-builder
	...
	- $(CONTAINER_TOOL) buildx rm daos-operator-builder
	rm Dockerfile.cross
...
CHART ?= charts/daos-operator
HELM ?= $(LOCALBIN)/helm

.PHONY: helm-sync
helm-sync: manifests ## Copy generated CRDs, the manager ClusterRole rules and the Grafana JSON into the chart.
	cp config/crd/bases/*.yaml $(CHART)/crds/
	cp config/grafana/DAOS-Grafana-Dashboard.json $(CHART)/dashboards/
	{ printf '{{- if .Values.rbac.create }}\n# GENERATED by `make helm-sync` from config/rbac/role.yaml (controller-gen). Do not edit.\napiVersion: rbac.authorization.k8s.io/v1\nkind: ClusterRole\nmetadata:\n  name: {{ include "daos-operator.fullname" . }}\n  labels:\n    {{- include "daos-operator.labels" . | nindent 4 }}\n'; \
	  sed -n '/^rules:/,$$p' config/rbac/role.yaml; printf '{{- end }}\n'; } > $(CHART)/templates/clusterrole.yaml

.PHONY: helm-lint
helm-lint: ## Lint and render the chart.
	$(HELM) lint $(CHART)
	$(HELM) template daos-operator $(CHART) --namespace daos-system --set system.create=true --set grafana.dashboard.enabled=true > /dev/null

.PHONY: helm-check
helm-check: helm-sync ## Fail when the chart is out of sync with the generated manifests.
	git diff --exit-code -- $(CHART)

.PHONY: helm-package
helm-package: helm-sync helm-lint ## Package the chart into dist/.
