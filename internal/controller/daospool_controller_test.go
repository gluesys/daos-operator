...
var _ = Describe("DaosPool / DaosContainer Controllers", func() {
	...
	It("creates a container with the agent sidecar Job and mirrors daos cont query", func() {
		...
		Expect(q.Command[2]).To(HaveSuffix("exec daos -j 'cont' 'query' 'p1' 'c1'"))
		...
		Expect(cr.Command[2]).To(ContainSubstring(`exec daos -j 'cont' 'create' 'p1' 'c1' '--type' 'POSIX' '--file-oclass' 'RP_2GX' '--dir-oclass' 'RP_2G1' '--chunk-size' '4194304' '--properties' 'rd_fac:1,cksum:crc32' '--acl-file' '/tmp/daos-acl'`))
